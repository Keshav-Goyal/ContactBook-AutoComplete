...
onload=function(){
    ...
    const del=document.getElementById("delete_input");
    ...
    add.onclick=function(){
        ...
        info.innerHTML+=details+"add to contact list<br>";
        contact_info.value="";
    };
    del.onclick=function(){
        let details = delete_info.value;
    ...
    let autocomplete=(inp)=>{
        ...
        let removeActive=(x)=>{
            for(let i=0;i<x.length;++i){
                x[i].classList.removeActive("active");
            }
        };
        let closeAllLists=(elemnt)=>{
            const x=document.getElementsByClassName("autocomplete-items");
            for(let i=0;i<x.length;i++){
                if(elemnt!==x[i]&& elemnt!==inp){
